...
extract(kng_get_posts_of_grid('post', [
    'source'   => $source,
    'orderby'  => $orderby,
    'order'    => $order,
    'limit'    => $limit,
...
$load_more = array(
    ...
    'thumbnail_size_custom'      => $widget->get_setting('thumbnail_custom', ''),
    // excerpt
...
<div <?php techrona_elementor_masonry_settings_render_attrs($widget, $settings,['class' => 'kng-post-grid layout-'.$layout]);?>>
    <?php if ($filter == "true"): ?>
        <div class="kng-grid-filter-wrap row gutters-40 justify-content-<?php echo esc_attr($filter_alignment); ?>">
    ...
    <?php if ($pagination_type == 'pagination') { ?>
        <div class="kng-post-pagination kng-grid-pagination" data-loadmore="<?php echo esc_attr(json_encode($load_more)); ?>" data-query="<?php echo esc_attr(json_encode($args)); ?>">
