...
// Heading
$widget->add_render_attribute( 'heading', 'class', 'kng-fancy-title text-uppercase font-700');
$widget->add_render_attribute( 'heading', 'class', 'text-'.$widget->get_setting('title_color', 'body'));
...
// Description
$widget->add_render_attribute( 'description', 'class', 'kng-fancy-description kng-heading h4 font-700');
$widget->add_render_attribute( 'description', 'class', 'text-'.$widget->get_setting('desc_color', 'heading'));
                ...
                switch ($widget->get_setting('icon_type','icon')) {
                    ...
                    case 'icon':
                        techrona_elementor_icon_render($settings,[
                            'class'        => 'kng-transition kng-fancy-icon text-40'
                        ]);
                    break;
                }
            ?></div>
        ...
        <div class="kng-fancybox-content kng-transition col">
            ...
            <div <?php kng_print_html($widget->get_render_attribute_string( 'heading' )); ?>>
                <?php if ( $link_attributes ) echo '<a '. implode( ' ', [ $link_attributes ] ).'>'; ?>
                    <?php kng_print_html($widget->get_setting('title')); ?>
            ...
            <div <?php kng_print_html($widget->get_render_attribute_string( 'description' )); ?>><?php
                kng_print_html($widget->get_setting('description'));
            ?></div>
            <?php endif; ?>
        </div>
    </div> 
