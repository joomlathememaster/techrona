...
$widget->add_render_attribute( 'counter', [
    'class' => 'kng-counter-number-wrap col-auto text-'.$widget->get_setting('number_color','heading')
] );
...
$widget->add_render_attribute( 'title', [
    'class' => 'kng-counter-title col text-'.$widget->get_setting('title_color','')
] );
...
$widget->add_render_attribute( 'description', [
    'class' => 'kng-counter-desc text-'.$widget->get_setting('description','body')
] );
...
<div <?php kng_print_html($widget->get_render_attribute_string( 'wrap' ));?>>
    <div class="kng-row-wrap row gutters-40 align-items-center">
        <div class="kng-counter-icon col-auto">
            <?php 
            if($settings['icon_type'] === 'icon'){
                techrona_elementor_icon_render($settings, [
                    'id'         => 'counter_icon',
                    'wrap_class' => 'kng-counter-icon text-'.$widget->get_setting('icon_text_color','accent'),
                    'default_icon'    => [
                        'value'   => '',
                        'library' => 'kngi'
                    ]
        ...
        <div class="kng-count-content col">
